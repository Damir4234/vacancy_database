...
def create_table():
    ...
    conn = psycopg2.connect(host="localhost", database="postgres",
                            user="postgres", password="050822")
    conn.autocommit = True
    cur = conn.cursor()

    cur.execute("DROP DATABASE IF EXISTS database_job")
    cur.execute("CREATE DATABASE database_job")

    conn.close()

    conn = psycopg2.connect(host="localhost", database="database_job",
                            user="postgres", password="050822")
...
def add_to_table(employers_list):
    ...
    with psycopg2.connect(host="localhost", database="database_job",
                          user="postgres", password="050822") as conn:
        with conn.cursor() as cur:
